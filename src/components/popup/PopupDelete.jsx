...
const PopupDelete = ({ openModal, onCancel, onDelete, title }) => {
  return (
    <>
      ...
      ></div>
      <div
        className={`w-[240px] rounded-[4px] bg-white shadow-lg fixed left-[50%] top-[10rem] right-[50%] translate-x-[-50%] py-[1.5rem] px-[1.5rem] z-[999] transition-all flex items-center justify-between flex-col ${
          openModal
      ...
      >
        <div className="text-center pt-[0.8rem]">
          <h2 className="text-[16px] font-bold">Are you sure to delete it</h2>
          <p className="text-[14px] mt-2">"{title}"</p>
        </div>

        <div className="flex items-center gap-2 text-[1.36rem] mt-6">
          <button
